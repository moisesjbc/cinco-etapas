...
func process(delta):
	...
	if death.global_position.distance_to(death.initial_global_position) > 30:
		all_in_position = false
		death.translate((death.initial_global_position - death.global_position).normalized() * 300 * delta)
	if all_in_position:
